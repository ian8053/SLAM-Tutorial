...
| 問題 | 決定因素 |
|-----|---------|
| Sensor 放哪裡？ | **物理意義**（量狀態 vs 量變化）|
| 相信哪個多？ | **Q、R 的設定**（雜訊大小）|

**舉例：爛 GPS + 強 IMU**

還是：
- IMU → 運動方程式（因為它量加速度 = 狀態變化）
- GPS → 觀測方程式（因為它量位置 = 狀態本身）

只是把 R（觀測雜訊）設很大，Kalman Gain 就會變小，結果更相信 IMU 的預測。

**舉例：強 RTK-GPS + 爛 IMU**

還是一樣的配置！只是把 R 設很小，Kalman Gain 變大，更相信 GPS。

**結論**：
- 放哪裡 = 看 sensor 量什麼（不能換）
- 相信多少 = 看 Q、R 怎麼設（可以調）

### 補充：如果兩個 Sensor 頻率一樣？

完全沒問題！就是**一直 Predict → Update → Predict → Update**
...
同頻運動 + 同頻觀測：
  σ: ↗↘↗↘↗↘↗↘↗↘
     每次 Predict 馬上 Update
```

同頻的話，不確定性一直維持在較低水平（因為馬上就被校正）。
