...
### `unique_lock<mutex> lock(mMutexMode);` 拆解
...
```cpp
// unique_lock = 獨佔鎖（一次只有一個人能持有）
unique_lock<mutex> lock(mMutex);

// shared_lock = 共享鎖（多人可以同時讀，寫的時候獨佔）
shared_lock<shared_mutex> lock(mMutex);
```

### 為什麼 ORB-SLAM3 用 unique_lock（獨佔鎖）？

1. **簡單**：不用區分讀和寫
2. **操作常常是讀+寫混合**：Tracking 讀地圖點 → 判斷 → 可能刪除某些點（不是純讀）
3. **鎖的時間很短**：等待時間短，shared_lock 優勢不大
...
**→ 解答：mpLocalMapper 指向的 LocalMapping 對象，執行它的 RequestStop() 函數。** 告訴 LocalMapping 線程：「停下來！」

```cpp
mpLocalMapper->RequestStop();
      ↑              ↑
   指標          這個對象執行 RequestStop()
```
...
**結論：`GrabImageStereo()` 本身就設計成「回傳 4x4 位姿矩陣」，跟輸入參數無關。**
...
**結論：空格只是分隔「回傳型態」和「函數名」，跟 `int add()` 的空格一樣！**
